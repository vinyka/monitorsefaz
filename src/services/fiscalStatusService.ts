...
// The API key is obtained exclusively from process.env.API_KEY as per the coding guidelines.
...
const createPromptForService = (service: ServiceType): string => {
    return `Por favor, simule o status de disponibilidade para o serviço ${service} em todas as Sefaz (Unidades Federativas e Sefaz Virtuais) do Brasil, em tempo real. A resposta deve ser uma lista em formato JSON.
...
export const fetchStatusData = async (service: ServiceType): Promise<SefazStatus[]> => {
    const prompt = createPromptForService(service);
    ...
    try {
        ...
        if (error instanceof Error) {
            throw new Error(`Falha ao buscar dados de ${service}. A comunicação com o serviço de IA falhou. Detalhe: ${error.message}`);
        }
        throw new Error(`Ocorreu um erro desconhecido ao buscar dados de ${service}.`);
    }
};
